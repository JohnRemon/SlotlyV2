...
@Service(StrategyType.MONTHLY_OCCURRENCES)
public class MonthlyOccurrencesStrategy implements RecurrenceStrategy {
    private final SlotUtils slotUtils;

    @Override
    public List<Slot> generateSlots(Event event, Schedule schedule) {
        return slotUtils.buildRecurringSlotsByOccurrences(event, schedule);
    }
}
