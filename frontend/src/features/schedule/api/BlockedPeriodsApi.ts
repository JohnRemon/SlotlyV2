...
export const getBlockedPeriods = async (): Promise<BlockedPeriod[]> => {
    const res = await API.get("/api/v1/schedule/blocked-periods");
...
export const getBlockedPeriodById = async (
    ...
    const res = await API.get(`/api/v1/schedule/blocked-periods/${id}`);
    return res.data.data;
};
...
export const createBlockedPeriod = async (
    ...
    const res = await API.post("/api/v1/schedule/blocked-periods", request);
    return res.data.data;
};

export const deleteBlockedPeriod = async (id: string) => {
    await API.delete(`/api/v1/schedule/blocked-periods/${id}`);
};
